...
import { downloadSpeakingTranscriptPdf } from "@/lib/speaking-transcript-pdf";
...
export function SpeakingResultSummary({
  ...
  const handleTranscriptDownload = () => {
    downloadSpeakingTranscriptPdf(result);
  };
  ...
  return (
    <section className="speaking-night speaking-result-summary grid gap-4 pb-1 lg:grid-cols-[minmax(0,1.48fr)_minmax(300px,1fr)] lg:gap-5">
      <article className="flex flex-col gap-3 rounded-[16px] border border-[#E5E7EB] bg-white p-4 shadow-[0_10px_32px_-28px_rgba(15,23,42,0.16)] lg:p-5">
        ...
        <div className="rounded-[16px] border border-[#E5E7EB] bg-[#FCFCFD] p-4 dark:border-slate-800 dark:bg-slate-900/70 lg:p-5">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <button
            ...
            className="inline-flex h-10 flex-1 items-center justify-center gap-1.5 rounded-[10px] border border-[#E5E7EB] bg-white text-sm font-semibold text-[#0F172A] transition hover:bg-[#F8FAFC] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
          >
        ...
        <div className="flex items-start gap-2 rounded-lg bg-[#F3EFFF] px-3 py-2.5 text-xs leading-5 text-[#7C3AED] sm:text-sm sm:leading-6">
          ...
          <span>Keep a copy of your transcript to review your answers and improve faster.</span>
        </div>
      </article>
  ...
  return (
    <div className="relative mx-auto flex h-[176px] w-[176px] items-center justify-center">
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={center} cy={center} r={normalizedRadius} fill="none" stroke="hsl(var(--secondary))" strokeWidth={stroke} />
        <circle
          cx={center}
          cy={center}
          r={normalizedRadius}
