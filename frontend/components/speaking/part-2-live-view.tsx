  ...
  return (
    <section className="speaking-night flex h-full min-h-0 flex-col overflow-hidden rounded-[18px] border border-[#E5E7EB] bg-white shadow-[0_12px_40px_-32px_rgba(15,23,42,0.16)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-none">
      ...
      <div className="grid min-h-0 flex-1 gap-3 p-3 lg:grid-cols-[minmax(0,1fr)_256px] lg:gap-4 lg:p-4">
        <article className="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)_auto] gap-2 rounded-[14px] border border-[#E5E7EB] bg-[#FCFCFD] p-3 dark:border-slate-800 dark:bg-slate-900/70 lg:gap-3 lg:p-4">
          ...
          >
            <label htmlFor="part-2-live-notes" className="text-sm font-semibold text-[#0F172A] dark:text-slate-50">
              Your notes (optional)
            </label>
            <div className="relative mt-1.5">
              <textarea
                ...
                className={cn(
                  "w-full resize-y rounded-[12px] border border-[#E5E7EB] bg-white px-3 py-3 text-sm leading-6 text-[#0F172A] outline-none transition placeholder:text-[#94A3B8] focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/15 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500",
                  compactExaminerStage ? "min-h-[88px]" : "min-h-[120px]",
                )}
              />
              <p className="pointer-events-none absolute bottom-3 right-3 text-xs font-medium text-[#94A3B8] dark:text-slate-500">
        ...
        <aside className="flex flex-col gap-2.5">
          ...
          <SidebarCard title="Stage">
            <p className="text-lg font-bold leading-snug text-[#0F172A] dark:text-slate-50">
              {viewPhase === "examiner" ? "Examiner introduction" : viewPhase === "preparation" ? "Preparation" : "Long turn"}
            </p>
          </SidebarCard>

          <SidebarCard title="Topic">
            <p className="text-lg font-bold leading-snug text-[#0F172A] dark:text-slate-50">{topicLabel || "Selected topic"}</p>
          </SidebarCard>
...
function ExaminerStage({
  ...
  return (
    <>
      ...
      <div className={cn("h-px w-full bg-[#E5E7EB] dark:bg-slate-800", compact ? "my-2" : "my-3")} />
      ...
      <section className="flex flex-col items-center text-center">
        <div className="relative flex w-full max-w-[460px] items-center justify-center">
          ...
          <div className={cn(
            "relative z-10 flex items-center justify-center rounded-full border border-[#A7F3D0] bg-white shadow-[0_0_0_7px_rgba(16,185,129,0.1),0_12px_24px_-16px_rgba(16,185,129,0.45)] dark:border-emerald-500/30 dark:bg-slate-900 dark:shadow-none",
            compact ? "h-[60px] w-[60px]" : "h-[72px] w-[72px]",
          )}>
            <span className={cn(
              "flex items-center justify-center rounded-full bg-[#ECFDF5] dark:bg-emerald-500/10",
        ...
        <h2 className={cn("font-semibold text-[#10B981] dark:text-emerald-400", compact ? "mt-1.5 text-sm sm:text-base" : "mt-2.5 text-base sm:text-lg")}>
          {userStatus}
        </h2>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <h2 className="text-lg font-semibold text-[#0F172A] dark:text-slate-50">Cue card</h2>
      </div>

      <div className="relative flex-1 rounded-[14px] border border-[#DDD6FE] bg-[#F3EFFF] px-6 py-6 dark:border-violet-500/30 dark:bg-violet-500/10">
        <button
          type="button"
          onClick={onListenAgain}
          className="absolute right-4 top-4 inline-flex h-9 items-center gap-1.5 rounded-[10px] border border-[#7C3AED] bg-white px-3 text-xs font-semibold text-[#7C3AED] transition hover:bg-[#FAF5FF] dark:border-violet-400/70 dark:bg-slate-900 dark:text-violet-300 dark:hover:bg-violet-500/10 sm:text-sm"
        >
          <Volume2 className="h-3.5 w-3.5" />
          Listen again
        </button>

        <p className="max-w-[92%] pr-2 text-base font-semibold leading-7 text-[#0F172A] dark:text-slate-50 sm:text-[17px]">
          {cueCard.promptText}
        </p>

        <p className="mt-5 text-sm font-semibold text-[#0F172A] dark:text-slate-50">You should say:</p>
        <ul className="mt-2 space-y-1.5 text-sm leading-6 text-[#334155] dark:text-slate-300">
...
export function Part2LiveLoadingState() {
  return (
    <div className="speaking-night flex flex-col items-center justify-center rounded-[20px] border border-[#E5E7EB] bg-white py-20 dark:border-slate-800 dark:bg-slate-950">
      ...
      <p className="mt-4 text-sm font-medium text-[#64748B] dark:text-slate-400">Connecting to examiner...</p>
    </div>
  );
}
...
function SidebarCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="rounded-[14px] border border-[#E5E7EB] bg-white p-3.5 shadow-[0_8px_24px_-20px_rgba(15,23,42,0.12)] dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
      <p className="text-xs font-semibold text-[#64748B] dark:text-slate-400 sm:text-sm">{title}</p>
