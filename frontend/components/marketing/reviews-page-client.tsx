...
export function ReviewsPageClient({ initialReviews }: ReviewsPageClientProps) {
  const { isAuthenticated, isPremium, name, phoneNumber, userId } = useAuthStore();
  ...
  useEffect(() => {
    void listPublicReviews().then(setReviews).catch(() => undefined);
  }, []);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (feedback.trim().length > 10 && band.trim().length > 0) {
              ...
              <CardContent className="p-6">
                {!isAuthenticated ? (
                  ...
                  <div className="text-center space-y-3 py-6 animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-muted-foreground">Your feedback has been submitted and is waiting for approval.</p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-foreground ml-1">Band score</label>
                      <input
                        value={band}
                        onChange={(event) => setBand(event.target.value)}
                        placeholder="7.5"
                        className="w-full h-11 rounded-xl border border-border/60 bg-muted/20 px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all placeholder:text-muted-foreground/50"
                        required
                      />
                    </div>
                    <div className="space-y-2">
                    ...
                    <Button type="submit" disabled={feedback.trim().length < 10 || band.trim().length === 0 || isSubmitting} className="w-full font-bold rounded-xl h-11 transition-all active:scale-95 group">
