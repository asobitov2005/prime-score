...
export function SpeakingTopicPickerClient() {
  ...
  const handleCardWheel = useCallback((event: ReactWheelEvent<HTMLElement>) => {
    if (!window.matchMedia("(min-width: 1024px)").matches) {
      return;
    }

    const topicsScroll = topicsScrollRef.current;
    if (!topicsScroll || topicsScroll.contains(event.target as Node)) {
      // Let the browser's native scrolling handle wheels over the topic grid.
      return;
    }

    const { scrollTop, scrollHeight, clientHeight } = topicsScroll;
    if (scrollHeight <= clientHeight + 1 || event.deltaY === 0) {
      return;
    }
    ...
    const delta = event.deltaMode === 1
      ? event.deltaY * 16
      : event.deltaMode === 2
        ? event.deltaY * clientHeight
        : event.deltaY;
    const nextScrollTop = Math.max(0, Math.min(scrollHeight - clientHeight, scrollTop + delta));

    if (nextScrollTop !== scrollTop) {
      event.preventDefault();
      topicsScroll.scrollTop = nextScrollTop;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <section
        ...
        className="flex min-h-0 flex-1 flex-col overflow-visible rounded-[18px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_-36px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-none sm:p-6 lg:overflow-hidden"
      >
            ...
            <div className="mt-6 flex min-h-0 flex-none flex-col lg:flex-1">
              ...
              <div
                ref={topicsScrollRef}
                className="mt-4 min-h-0 flex-none touch-pan-y overflow-visible pb-1 pr-1 [-ms-overflow-style:none] [scrollbar-width:thin] lg:flex-1 lg:overflow-y-auto lg:overscroll-contain lg:[scrollbar-gutter:stable] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-200 dark:[&::-webkit-scrollbar-thumb]:bg-slate-700"
              >
                {loading ? (
            ...
            <div className="sticky bottom-0 z-10 mt-4 flex shrink-0 flex-col gap-3 border-t border-slate-200 bg-white/95 pt-4 backdrop-blur-sm sm:flex-row dark:border-slate-800 dark:bg-slate-950/95">
              <button
                type="button"
