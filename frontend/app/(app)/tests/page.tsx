              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    card.title === "Feedback & Analytics" && "hidden sm:flex",
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
            <div className="flex min-w-0 flex-row items-start gap-3 sm:items-stretch sm:gap-4">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="line-clamp-1 text-sm font-semibold text-slate-950 dark:text-slate-50 sm:text-base">{continueTitle}</h2>
                <div className="mt-0.5 flex flex-col gap-0.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                  <p className="text-xs text-slate-500 dark:text-slate-400 sm:text-sm">Reading · {continueProgress}% completed</p>
                  <p className="shrink-0 text-xs font-medium text-slate-500 dark:text-slate-400 sm:text-sm">{continueAnswered} · {continueSpent} spent</p>
                </div>
                <div className="mt-2">
          ...
          <div className="grid grid-cols-3 gap-2.5 sm:gap-4">
            {collectionCards.map((card) => (
              <Link
                key={card.title}
                href={card.href}
                className="flex flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 text-center shadow-[0_8px_18px_-18px_rgba(15,23,42,0.14)] transition-colors hover:border-orange-200 hover:bg-orange-50/20 dark:border-slate-800 dark:bg-slate-900/70 dark:shadow-none dark:hover:border-orange-500/30 dark:hover:bg-orange-500/10 sm:h-[6.5rem] sm:flex-row sm:items-center sm:gap-4 sm:p-5 sm:text-left"
              >
                <CollectionImageTile
                  src={card.imageSrc}
                  alt={card.imageAlt}
                  className="h-[3.5rem] w-[2.625rem] shrink-0 sm:h-[4.5rem] sm:w-[3.375rem]"
                />
                <span className="min-w-0 flex-1">
                  <span className="block text-xs font-semibold leading-snug text-slate-950 dark:text-slate-50 sm:text-base">{getCollectionTitle(card.title)}</span>
                  <span className="mt-1 block text-[11px] leading-tight text-slate-500 dark:text-slate-400 sm:text-sm">{card.subtitle}</span>
                </span>
