...
export function PremiumDashboardSpotlight() {
  ...
  return (
      <Card className="w-full min-h-[230px] rounded-3xl border-primary/20 bg-gradient-to-br from-primary/10 via-card to-card shadow-sm">
        <CardContent className="flex h-full flex-col justify-between p-4">
          <div>
            <div>
              <p className="bg-gradient-to-r from-primary/90 via-primary to-primary/70 bg-clip-text text-base font-black tracking-tight text-transparent">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2.5">
            {[
              ...
              <div key={item.label} className="rounded-2xl border border-primary/15 bg-background/85 px-3 py-3 text-center shadow-sm">
                <p className="text-xl font-black tracking-tight text-foreground">{item.value}</p>
                <p className="mt-1 text-[10px] font-black uppercase tracking-[0.22em] text-muted-foreground">{item.label}</p>
              </div>
            ))}
          </div>

          <Button asChild className="mt-3 h-9 w-full rounded-xl bg-primary text-primary-foreground font-bold shadow-lg shadow-primary/20 hover:bg-primary/90">
