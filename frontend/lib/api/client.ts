...
export interface FavoriteTestResponse {
  test_id: string;
  title: string;
  test_type: TestType;
...
export function createApiClient(config: ApiClientConfig = {}) {
  ...
  return {
    ...
    getFavorites: () =>
      request<FavoriteTestResponse[]>("/me/favorites").then((data) => ({ data })),
    addFavorite: (testId: string) =>
      request<{ message: string }>(`/me/favorites/${testId}`, { method: "POST", body: JSON.stringify({}) }),
    removeFavorite: (testId: string) =>
      request<{ message: string }>(`/me/favorites/${testId}`, { method: "DELETE" }),
